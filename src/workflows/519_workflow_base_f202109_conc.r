...
TS_strategy_base9 <- function( pinputexps )
{
  ...
  param_local$train$testing <- c(202109)
...
HT_tuning_base <- function( pinputexps, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    # Hiperparámetros optimizables con límites permitidos
    max_depth = c(-1L, 64L, "integer"), # -1 para sin límite, hasta 64 según documentación
    min_gain_to_split = c(0.0, 15.0), # Ganancia mínima para un split, de 0 a 15
    min_sum_hessian_in_leaf = c(0.00001, 100000), # Peso mínimo de hessian en hoja, de 1e-5 a 1e5
    lambda_l1 = c(0.0, 100.0), # Regularización L1, de 0 a 100
    lambda_l2 = c(0.0, 100.0), # Regularización L2, de 0 a 100
    
    # Estos parámetros permanecen fijos
    max_bin = 31L,
    num_iterations = 9999, # Gran valor limitado por early_stopping_rounds
    
    # Parámetros adicionales optimizables con límites documentados
    bagging_fraction = c(0.0, 1.0), # Fracción de datos seleccionados, de 0 a 1
    pos_bagging_fraction = c(0.0, 1.0), # Fracción de datos positivos, de 0 a 1
    neg_bagging_fraction = c(0.0, 1.0), # Fracción de datos negativos, de 0 a 1
    is_unbalance = FALSE, # Si el dataset está balanceado
    scale_pos_weight = c(0.001, 1000), # Peso de la clase positiva, de 0.001 a 1000
    
    drop_rate = c(0.0, 1.0), # Tasa de dropout, de 0 a 1
    max_drop = c(0L, 50L, "integer"), # Máximo de dropouts permitidos, hasta 50
    skip_drop = c(0.0, 1.0), # Fracción a saltar en dropout, de 0 a 1
    
    extra_trees = FALSE, # Si se usan árboles extra o no
    
    # Configuración de parámetros de aprendizaje y features
    learning_rate = c(0.001, 1.0), # Tasa de aprendizaje, de 0.001 a 1
    feature_fraction = c(0.0, 1.0), # Fracción de features, de 0 a 1
    num_leaves = c(1L, 131072L, "integer"), # Número de hojas, de 1 a 131072
    min_data_in_leaf = c(0L, 1000000L, "integer") # Mínimo de datos requeridos en una hoja, de 0 a 1e6
  )
